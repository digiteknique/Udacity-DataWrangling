...
def audit_file(filename):
    ...
    for event, elem in ET.iterparse(filename, events=("start",)):
        ...
        if elem.tag == 'tag' and 'k' in elem.attrib:
            count_unique(elem.get('k'), audit_data['unique_k_tags'])
    ...
    sorted_tags = sorted(audit_data['unique_k_tags'].items(), key=operator.itemgetter(1)) 
    sorted_tags.reverse()    
    audit_data['unique_k_tags'] = sorted_tags
    return audit_data

if __name__ == '__main__':
    # Audit the file
    audit_data = audit_file('norman.osm')
    print('Count of unique tags in the file: {}'.format(len(audit_data['unique_tags'])))
    print('Count of unique k values in the file: {}'.format(len(audit_data['unique_k_tags'])))
    ...
    pprint.pprint(audit_data['unique_k_tags'])
